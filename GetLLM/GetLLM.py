...
import algorithms.newphase
import algorithms.beta
import algorithms.newbeta
...
# _parse_args()-function
#===================================================================================================
def _parse_args(start_args=None):
...
def main(accelerator,
    ...
    try:
        ...
        #-------- START Phase
        phase_d, tune_d = algorithms.phase.calculate_phase(getllm_d, twiss_d, tune_d, mad_twiss,
                                                           mad_ac, mad_elem, temp_dict)

        #-------- START Total Phase
        newphase_d, newtune_d = algorithms.newphase.calculate_phase(getllm_d, twiss_d, tune_d, files_dict)

        #-------- START Beta
        beta_d = algorithms.newbeta.calculate_beta_from_phase(getllm_d, twiss_d, newtune_d, newphase_d, files_dict)
        ...
        for tfsfile in files_dict.itervalues():
            try:
                ...
                LOGGER.error('Error happened while writing tfs file "{}"'
                             .format(tfsfile.get_file_name()))
...
def _analyse_src_files(getllm_d, twiss_d, files_to_analyse, nonlinear, turn_by_turn_algo, files_dict, use_average, calibration_twiss):
    ...
    for file_in in files_to_analyse.split(','):
        ...
        twiss_file_x = None
        try:
            twiss_file_x = tfs_pandas.read_tfs(file_x, index="NAME")
        ...
        twiss_file_y = None
        try:
            twiss_file_y = tfs_pandas.read_tfs(file_y, index="NAME")
    ...
    model = getllm_d.accelerator.get_model_tfs()
    ...
    twiss_d.non_zero_dpp_commonbpms_x, twiss_d.non_zero_dpp_unionbpms_x = _get_commonbpms(
        twiss_d.non_zero_dpp_x, model, union, "H")
    twiss_d.non_zero_dpp_commonbpms_y, twiss_d.non_zero_dpp_unionbpms_y = _get_commonbpms(
        twiss_d.non_zero_dpp_y, model, union, "V")
...
def _start():
    ...
    main(accelerator,
         ...
         model_filename=os.path.join(accelerator.model_dir, "twiss.dat"),
         accel=options.accel,
